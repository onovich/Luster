...
function draw(){
 ...
 for(const r of renderers){r.setParameters(params);r.render({angle:pose.angle,inspect:inspection});}
...
$('stage').onpointermove=e=>{if(mode||!ready)return;const rect=$('stage').getBoundingClientRect(),next=e.clientX<rect.left+rect.width/2?-4:4;if(pose.target!==next)go(next);};
$('stage').onpointerleave=()=>{if(!mode&&ready)go(0);};
...
async function init(){
 for(let i=0;i<8;i++){
  ...
  const canvas=document.createElement('canvas');canvas.id=`pocket${i}`;slot.append(canvas);$('pockets').append(slot);$('pocket').add(new Option(String(i+1),i));
  const response=await fetch(new URL(`./assets/normals/normal-${i}.rgba`,import.meta.url));if(!response.ok)throw new Error('Normal unavailable');
  const data=new Uint8Array(await response.arrayBuffer()),background=new Image();background.src=new URL(`./assets/images/base-${i}.png`,import.meta.url);await background.decode();
  renderers.push(await FoilRenderer.create(canvas,{normal:{data,width:512,height:512},background}));
 }
 ready=true;draw();$('status').textContent='8 / 8';
}
// Small inspectable demo API, also used by regression tests. It is not the renderer API.
window.foilDemo={get renderers(){return renderers;},state:()=>({ready,variant,parameters:{...params},angle:pose.angle,target:pose.target,active:pose.active,drawCount,errors,glErrors:renderers.map(r=>r.gl.getError())}),setAngle(angle){stop();pose.set(angle);draw();},go,restore,patch,capture(){draw();return renderers.map(r=>r.canvas.toDataURL());}};
