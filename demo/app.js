import {LayeredRenderer} from '../src/webgl/layered-renderer.js';
import {cardSurfaces,makeCardSurface} from './card-surfaces.js';
...
function draw(){
 ...
 for(let i=0;i<renderers.length;i++){const r=renderers[i];if(!r||$('view').value==='material'&&i!==+$('pocket').value)continue;cardMotions[i]?.apply(r);r.setParameters(params);r.render({angle:pose.angle,inspect:inspection});}
