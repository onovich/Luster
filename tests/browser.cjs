...
(async()=>{
 ...
 await page.locator('#view-material').click();for(let i=0;i<8;i++){await page.locator('.material-thumb').nth(i).click();assert.equal(await page.locator('.pocket:visible').count(),1);}await page.locator('.material-thumb').nth(0).click();await page.screenshot({path:path.join(out,'material-b14.png'),fullPage:true});await page.locator('#view-book').click();
