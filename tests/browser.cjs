...
(async()=>{
 ...
 const lifted=await page.evaluate(()=>({offset:foilDemo.state().cardOffsets[0],canvasTop:document.querySelector('#slot0 canvas').offsetTop,content:foilDemo.renderers[0].cardPose.lift}));
 assert.ok(lifted.canvasTop<0);assert.ok(lifted.content>0);
