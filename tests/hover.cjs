...
(async()=>{const b=await chromium.launch(launchOptions);try{
 ...
 const geometry=await p.evaluate(()=>{const r=foilDemo.renderers[2],g=r.gl,a=new Uint8Array(r.canvas.width*r.canvas.height*4);r.render({angle:0});g.readPixels(0,0,r.canvas.width,r.canvas.height,g.RGBA,g.UNSIGNED_BYTE,a);const row=y=>{const xs=[];for(let x=0;x<r.canvas.width;x++)if(a[(y*r.canvas.width+x)*4+3])xs.push(x);return {min:xs[0],max:xs.at(-1),width:xs.length};};let clipped=false;for(let y=r.baseHeight+1;y<r.canvas.height;y++)if(a[(y*r.canvas.width)*4+3]||a[(y*r.canvas.width+r.canvas.width-1)*4+3])clipped=true;return {rows:[row(565),row(610)],clipped};});
 assert.equal(geometry.clipped,false);assert(geometry.rows.every(r=>r.min>0&&r.max<487&&r.width>300),JSON.stringify(geometry));assert(Math.abs(geometry.rows[0].width-geometry.rows[1].width)>2,'Projected edges must converge');
 ...
 await p.screenshot({path:'.test-output/material-fit-large.png',fullPage:true});assert.deepEqual(errors,[]);console.log('PASS: no startup lift, exclusive hover, leave/blur/reload reset, converging unclipped projection, proportional desktop UI',geometry);
