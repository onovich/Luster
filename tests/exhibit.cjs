...
(async()=>{const b=await chromium.launch(launchOptions);try{
 ...
 await p.locator('#advanced summary').click();await p.waitForFunction(()=>!foilDemo.state().switching);await p.evaluate(()=>foilDemo.restore('B14'));await p.waitForFunction(()=>foilDemo.state().variant==='B14');assert.equal(await p.locator('#schemeName').textContent(),'B14');await p.keyboard.press('Escape');
