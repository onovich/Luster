...
(async()=>{
 ...
 try{
  for(const width of [320,390,768,1440]){
   ...
   assert.equal(await page.locator('#period').isVisible(),false);
   assert.equal(await page.locator('[data-effect]:visible').count(),4);
   const menu=await page.locator('.preset-popover').boundingBox();assert.ok(menu.x>=0&&menu.x+menu.width<=width);
